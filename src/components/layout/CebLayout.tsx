...
const navItems = [
  { to: '/ceb/dashboard', label: '🏠 Dashboard', exact: true },
  { to: '/ceb/conselheiros', label: '👥 Conselheiros' },
  { to: '/ceb/dizimistas', label: '📋 Dizimistas' },
  { to: '/ceb/doacoes', label: '💰 Doações' },
]

export default function CebLayout({ children }: { children: ReactNode }) {
    ...
    <div style={{ display: 'flex', minHeight: '100vh', background: 'var(--bg)' }}>
      <nav style={{
        ...
        <div style={{ padding: '0 20px 24px', borderBottom: '1px solid var(--border)', marginBottom: 8 }}>
          ...
          <div style={{ fontSize: 11, color: 'var(--text-muted)', marginTop: 2 }}>CEB</div>
        </div>
        {navItems.map(item => {
